(function () {
  ...
  function injectSafeFixStyles() {
    if (document.getElementById("ktws-safe-fix-styles")) return;
    const style = document.createElement("style");
    style.id = "ktws-safe-fix-styles";
    style.textContent = `
      /* Homepage Now Hiring restore only if missing */
      .ktws-hiring-restore {
        background:
      ...
      /* Jerry Lee Vaughan memorial image fix — memorial only */
      .memorial-photo-card {
        background:
          radial-gradient(circle at top right, rgba(240,189,47,.12), transparent 34%),
          #0d131b !important;
      }
      ...
      /* Medical courier image crop fix — targets courier card images only */
      img[src*="courier.jpg"],
      img[src*="courier-secondary.jpg"],
      img[alt*="Medical Courier"],
      img[alt*="medical courier"] {
        object-fit: cover !important;
        object-position: center 38% !important;
      }

      /* Travis leadership image fix — targets Travis card only via JS-added class */
  ...
  function applyTargetedImageFixes() {
    /* Fix Travis leadership photo by finding the card that actually contains his name */
    document.querySelectorAll(".team-card, .featured-team-card, article, .owner-card").forEach((card) => {
      const text = (card.innerText || "").toLowerCase();
      if (text.includes("travis lewter") || text.includes("founder & chief executive officer") || text.includes("founder and chief executive officer")) {
        const img = card.querySelector("img");
        if (img) img.classList.add("ktws-travis-photo-fix");
      }
    });

    /* Fix the Medical Courier division card without touching other images */
    document.querySelectorAll(".showcase-card, .card, article").forEach((card) => {
      const text = (card.innerText || "").toLowerCase();
      if (text.includes("medical courier") || text.includes("healthcare logistics")) {
        const img = card.querySelector("img");
        if (img) {
